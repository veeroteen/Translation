...
void LR1()
{
   ...
   StaticTable<KEYWORDS> keywords;
   {
      std::ifstream stream(testFolder + "Keywords.txt");
      std::string buff = "";
      std::vector<Fundamentals<KEYWORDS>> bArr;
      while (std::getline(stream, buff))
      {
         std::pair<std::string, KEYWORDS> par;
         divide(par,buff);
         bArr.emplace_back(par.first,par.second,TokenType::KEYWORD);
      }
      keywords.load(std::move(bArr));
   }
   StaticTable<OPERATORS> operators;
   {
      std::ifstream stream(testFolder + "Operators.txt");
      std::string buff = "";
      std::vector<Fundamentals<OPERATORS>> bArr;
      while (std::getline(stream, buff))
      {
         std::pair<std::string, OPERATORS> par;
         divide(par, buff);
         bArr.emplace_back(par.first, par.second, TokenType::OPERATOR);
      }
      operators.load(std::move(bArr));
   }
   StaticTable<SEPARATORS> separators;
   {
      std::ifstream stream(testFolder + "Separators.txt");
      std::string buff = "";
      std::vector<Fundamentals<SEPARATORS>> bArr;
      while (std::getline(stream, buff))
      {
         std::pair<std::string, SEPARATORS> par;
         divide(par, buff);
         bArr.emplace_back(par.first, par.second, TokenType::SEPARATOR);
      }
